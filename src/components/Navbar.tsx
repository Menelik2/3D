...
export function Navbar({ logoUrl }: { logoUrl?: string }) {
  ...
  return (
    <>
      ...
      {/* Backdrop */}
      <div
        className={`fixed inset-0 z-[55] md:hidden bg-black/70 backdrop-blur-sm transition-opacity duration-400 ${
      ...
      {/* Left sidebar panel — design match */}
      <aside
        className={`fixed top-0 left-0 z-[56] md:hidden flex h-full w-[min(300px,86vw)] flex-col bg-[#0a0a0a] transition-transform duration-400 ease-out ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
        style={{
          boxShadow: open ? "8px 0 40px rgba(225, 29, 72, 0.12)" : "none",
        }}
        aria-hidden={!open}
      >
        {/* Red neon edge */}
        ...
        />
        <div
          className="pointer-events-none absolute bottom-0 left-0 h-16 w-[2px]"
          style={{
            background: "#e11d48",
            boxShadow: "0 0 16px rgba(225,29,72,0.8)",
          }}
        />
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-2">
              ...
              return (
                <li key={link.href + link.label}>
                  <Link
                    href={link.href}
                    onClick={() => setOpen(false)}
                    className={`group flex items-center gap-3.5 rounded-sm px-4 py-3.5 text-[11px] font-medium uppercase tracking-[0.22em] transition-all duration-200 ${
                      active
          ...
          <div className="mt-6 px-4">
            <Link
              href="/start-a-project"
              onClick={() => setOpen(false)}
              className="flex w-full items-center justify-center bg-accent px-4 py-3.5 text-[10px] font-medium uppercase tracking-[0.2em] text-white transition hover:bg-accent-hover"
            >
              Start a Project
            </Link>
          </div>
