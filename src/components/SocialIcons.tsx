...
type Props = {
  social: SocialConfig;
  /** Show all five platforms even if URL empty (uses #) */
  showEmpty?: boolean;
  className?: string;
    ...
    <div className={`flex flex-wrap items-center gap-2.5 ${className}`}>
      {links.map((item) => (
        <a
          ...
          className={`${pad} inline-flex items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white/55 transition-all duration-300 hover:border-accent/50 hover:bg-accent/15 hover:text-accent hover:scale-105 ${item.disabled ? "opacity-40 cursor-default" : ""} ${iconClassName}`}
        >
          {icons[item.key]}
        </a>
      ))}
